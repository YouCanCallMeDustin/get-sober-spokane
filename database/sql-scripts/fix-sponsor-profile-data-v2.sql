-- Fix sponsor profile data linking (Version 2)
...
begin
    if not exists (select 1 from information_schema.tables where table_name = 'profiles_consolidated') then
        ...
        -- Add recovery_program column if it doesn't exist
        if not exists (
            select 1 from information_schema.columns 
            where table_name = 'profiles_consolidated' 
            and column_name = 'recovery_program'
        ) then
            alter table profiles_consolidated add column recovery_program text;
            raise notice 'Added recovery_program column to profiles_consolidated.';
        end if;
    end if;
end $$;
...
begin
    ...
    loop
        ...
        if not exists (select 1 from profiles_consolidated where user_id = sponsor_record.user_id) then
            ...
            -- Update existing profile with sponsor data (only update existing columns)
            ...
            -- Update recovery_program if the column exists
            if exists (
                select 1 from information_schema.columns 
                where table_name = 'profiles_consolidated' 
                and column_name = 'recovery_program'
                ...
                update profiles_consolidated 
                set recovery_program = coalesce(sponsor_record.recovery_program, recovery_program)
                where user_id = sponsor_record.user_id;
            end if;
