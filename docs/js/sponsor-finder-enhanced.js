// Enhanced Sponsor Finder with Google Auth Integration
// This version ensures sponsor cards pull data from Google Auth profiles
...
class SponsorFinder {
  ...
  async loadAvailableSponsors() {
    try {
      ...
      // Load profiles for these users with Google Auth data
      ...
      // Also get Google Auth data for users
      ...
      if (authError) {
        console.warn('Could not load auth users:', authError);
      }

      // Combine sponsor data with profile data and Google Auth data
      this.availableSponsors = sponsors.map(sponsor => {
        const profile = profiles?.find(p => p.user_id === sponsor.user_id);
        const authUser = authUsers?.find(u => u.id === sponsor.user_id);
        ...
        if (authUser?.raw_user_meta_data) {
          googleName = authUser.raw_user_meta_data.full_name || 
                      authUser.raw_user_meta_data.name || 
                      authUser.raw_user_meta_data.display_name;
          googleAvatar = authUser.raw_user_meta_data.avatar_url || 
                        authUser.raw_user_meta_data.picture;
        }
        
        // Use Google Auth data as fallback
        const displayName = profile?.display_name || googleName || 'Anonymous Sponsor';
        const avatarUrl = profile?.avatar_url || googleAvatar;
        const bio = profile?.bio || sponsor.sponsor_bio || 'Committed to helping others in recovery';
      ...
      console.log('Loaded sponsors with Google Auth data:', this.availableSponsors.length);
  ...
  filterSponsors() {
    ...
    if (this.currentFilters.yearsSober) {
      const years = parseInt(s.years_sober || 0);
        ...
        <div class="card sponsor-card h-100">
          <div class="card-body text-center">
            <div class="sponsor-avatar">
              ${profile.avatar_url ? 
                `<img src="${profile.avatar_url}" alt="${profile.display_name}" style="width: 100%; height: 100%; border-radius: 50%; object-fit: cover;" onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
  ...
  setupEventListeners() {
    ...
    console.log('Enhanced sponsor finder event listeners set up');
  }
