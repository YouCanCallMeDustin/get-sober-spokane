// Generate Safe Forum Upgrades Script
// This script creates a safe upgrade script based on your existing database structure

const fs = require('fs');
const path = require('path');

class SafeForumUpgradeGenerator {
    constructor() {
        this.script = '';
        this.addHeader();
    }
    ...
    addHeader() {
        this.script += `-- Safe Forum Upgrades Script
...
`;
    ...
    addTableChecks() {
        this.script += `-- Check what tables exist before proceeding
DO $$
DECLARE
...
// Generate the script
const generator = new SafeForumUpgradeGenerator();
const script = generator.generate();

// Write to file
const outputPath = path.join(__dirname, '..', 'database', 'sql-scripts', 'forum-upgrades-safe.sql');
fs.writeFileSync(outputPath, script);

console.log('✅ Generated safe forum upgrade script!');
console.log(`📁 File saved to: ${outputPath}`);
console.log('\n📝 This script will:');
console.log('   - Only add new features without conflicts');
console.log('   - Check for existing tables before creating');
console.log('   - Check for existing policies before creating');
console.log('   - Preserve all existing data and structure');
console.log('\n🚀 Run this script in your Supabase SQL Editor to upgrade your forum!');
