-- Safe Table Update Script for Get Sober Spokane Dashboard
...
BEGIN
    ...
    SELECT column_name INTO milestone_user_col
    FROM information_schema.columns
    WHERE table_schema = 'public' 
    ...
    -- Create RLS policies for profiles table using the correct column name
    IF profile_id_col IS NOT NULL THEN
        ...
        RAISE NOTICE 'Created RLS policies for profiles table using column: %', profile_id_col;
    ELSE
        RAISE NOTICE 'Could not determine primary key column for profiles table. RLS policies not created.';
    END IF;

    -- Create RLS policies for recovery_milestones table using the correct column name
    IF milestone_user_col IS NOT NULL THEN
        ...
        EXECUTE format('CREATE POLICY "Users can update own milestones" ON recovery_milestones FOR UPDATE USING (auth.uid()::text = %I)', milestone_user_col);
        EXECUTE format('CREATE POLICY "Users can delete own milestones" ON recovery_milestones FOR DELETE USING (auth.uid()::text = %I)', milestone_user_col);
        
        RAISE NOTICE 'Created RLS policies for recovery_milestones table using column: %', milestone_user_col;
    ELSE
