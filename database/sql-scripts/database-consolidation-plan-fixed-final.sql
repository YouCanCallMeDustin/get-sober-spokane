-- Database Consolidation Plan for Sober Spokane (FINAL FIXED)
...
-- Check the actual structure of each table before migration
SELECT 'profiles_consolidated STRUCTURE:' as info;
SELECT 
    column_name,
    data_type,
    is_nullable
...
SELECT 'forum_users STRUCTURE:' as info;
SELECT 
    column_name,
    data_type,
    is_nullable
...
-- =====================================================
-- STEP 2: DROP EXISTING VIEWS FIRST
-- =====================================================

-- Drop existing views to avoid column rename conflicts
...
-- =====================================================
-- STEP 3: CONSOLIDATE PROFILE TABLES
-- =====================================================
...
SELECT 'profiles_unified table created successfully' as status;

-- =====================================================
-- STEP 4: MIGRATE DATA FROM EXISTING TABLES (SAFE VERSION)
-- =====================================================
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'profiles_consolidated') THEN
        ...
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles_consolidated' AND column_name = 'user_id') THEN
        ...
        ELSE
            RAISE NOTICE 'profiles_consolidated table exists but does not have user_id column';
        END IF;
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_profiles') THEN
        ...
        ELSE
            RAISE NOTICE 'user_profiles table exists but does not have user_id column';
        END IF;
    END IF;
END $$;

-- Migrate data from forum_users (if it exists and has data) - SAFE VERSION
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'forum_users') THEN
        -- Check if the table has the expected columns
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'forum_users' AND column_name = 'user_id') THEN
            INSERT INTO profiles_unified (
        ...
        ELSE
            RAISE NOTICE 'forum_users table exists but does not have user_id column - checking for alternative column names';
            
            -- Try to find alternative column names
            IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'forum_users' AND column_name = 'id') THEN
                RAISE NOTICE 'forum_users has id column instead of user_id';
            END IF;
        END IF;
    END IF;
END $$;
...
-- =====================================================
-- STEP 5: CREATE UNIFIED VIEWS (AFTER DROPPING OLD ONES)
-- =====================================================
...
-- =====================================================
-- STEP 6: UPDATE FUNCTIONS AND TRIGGERS
-- =====================================================
...
SELECT 'All profile tables have been consolidated into profiles_unified' as message;
SELECT 'All views now point to the unified table' as message;
SELECT 'Your existing code should work without changes' as message;
